import { Col, Container, Image, Row } from "react-bootstrap";
import styles from "./WorksSection.module.css";
const worksData = [
  {
    id: 1,
    link: "https://www.google.com",
    image: require("../../assests/images/img2.jpg"),
    title: "Lonely Path",
    subtitle: "Web Design",
  },
  {
    id: 2,
    link: "https://www.google.com",
    image: require("../../assests/images/img3.jpg"),
    title: "Photographer Girl",
    subtitle: "Branding",
  },
  {
    id: 3,
    link: "https://www.google.com",
    image: require("../../assests/images/img4.jpg"),
    title: "The Difference",
    subtitle: "Web Design",
  },
  {
    id: 4,
    link: "https://www.google.com",
    image: require("../../assests/images/img5.jpg"),
    title: "Nature Patterns",
    subtitle: "Branding",
  },
  {
    id: 5,
    link: "https://www.google.com",
    image: require("../../assests/images/img6.jpg"),
    title: "The Difference",
    subtitle: "Photography",
  },
  {
    id: 6,
    link: "https://www.google.com",
    image: require("../../assests/images/img7.jpg"),
    title: "Winter Sonata",
    subtitle: "Web Design",
  },
  {
    id: 7,
    link: "https://www.google.com",
    image: require("../../assests/images/img8.jpg"),
    title: "Lonely Path",
    subtitle: "Branding",
  },
  {
    id: 8,
    link: "https://www.google.com",
    image: require("../../assests/images/img9.jpg"),
    title: "Appreciation",
    subtitle: "Photography",
  },
  {
    id: 9,
    link: "https://www.google.com",
    image: require("../../assests/images/img2.jpg"),
    title: "Happy Days",
    subtitle: "Web Design",
  },
];
const WorksSection = () => {
  return (
    <section id="work" className={styles.workBlock}>
      <Container className={styles.container}>
        <div className={styles.titleHolder}>
          <h2>Our Works</h2>
          <div className={styles.subTitle}>our owesome work</div>
        </div>
        <Row className={styles.portfoliolist}>
          {worksData.map((works) => {
            return (
              <Col sm={6} xxl={4} xl={4} lg={4} md={6} key={works.id}>
                <div className={styles.portfolioWrapper}>
                  <a href={works.link}>
                    <Image src={works.image} className={styles.img} />
                    <div className={`${styles.label} ${"text-center"}`}>
                      <h3>{works.title}</h3>
                      <p>{works.subtitle}</p>
                    </div>
                  </a>
                </div>
              </Col>
            );
          })}
        </Row>
      </Container>
      <hr />
    </section>
  );
};

export default WorksSection;
